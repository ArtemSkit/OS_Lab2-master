...
//				Iterator temp = stackMemory.iterator();
                for (Vertex vertex : stackMemory) {
...
//        System.out.println("Returning stack object " + stackObject);
        return stackObject;
    }

    public Vertex getHeapObject(String id) {
        int heapkSize = heapUse();

        // Checks the entire stackMemory to see if id is within it
        // By taking the front and moving it to the back.
        if (heapkSize > 0) {
            for (int i = 0; i < heapkSize; i++) {
                Vertex temp = heapMemory.getVertex(i + "");
...
//            System.out.println("objectQueue.isEmpty " + objectQueue.isEmpty());
            // Runs until all elements in the stackMemory are checked for references.
            while (!objectQueue.isEmpty()) {
                Vertex obj1 = objectQueue.remove();
...
//            System.out.println("markedObjects " + markedObjects.values());
        }
    }
